...
<body class="bg-white flex flex-col items-center min-h-screen">
    ...
    <div class="relative w-[95%] max-w-[970px] h-auto md:h-[520px] rounded-2xl overflow-hidden shadow-2xl bg-white">
        ...
        <!-- CARD LOGIN -->
        <div
            class="relative md:absolute md:top-6 md:right-6 md:bottom-6 bg-[#13810A] bg-opacity-65 md:backdrop-blur-md p-8 rounded-2xl shadow-2xl w-full md:w-[360px] text-white flex flex-col justify-center items-center transition-all duration-300">
            <div class="flex flex-col items-center w-full">
                ...
                <form action="{{ route('login') }}" method="POST" class="w-full flex flex-col gap-4">
                    ...
                    @csrf
                    @if ($errors->has('login'))
                        <div class="bg-red-100 text-red-700 px-3 py-2 rounded-md mb-3 text-center">
                            {{ $errors->first('login') }}
                        </div>
                    @endif
                    <div>
